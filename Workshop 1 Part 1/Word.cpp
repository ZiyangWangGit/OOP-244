...
namespace sdds {
    ...
    int readWord(char* word, FILE* fptr, int maxLen) {    //reads characters from a file
        int ch;
        int i = 0;
        do {
            ch = fgetc(fptr);
            word[i++] = ch;
        } while (ch != EOF && !isSpace(ch) && i < maxLen);//loop will continue if its not EOF or space or exceed the maxlen 
        word[i] = '\0';
        return !(i == 1 && ch == EOF);                  //return true if word successfully read
    }

    void addWord(Word words[], int* index, const char newWord[]) {
        strCpy(words[*index].letters, newWord);        //copy the newWord to words.letter at the current index
        words[*index].count++;
        (*index)++;
    }

    void title(const char* value, int len) { // uses iomanip header file //print a formatted table
        cout << value << endl;                          
    ...
    void endList() {
        cout << "------------------------------------------------" << endl;//display dashes to indicate it is the ned of list
    }
    ...
    void print(const Word* w, int gotoNextLine, int len) { // uses iomanip header file //display data from Word structure
            ...
            for (j = 0; j < cnt - i - 1; j++) {
                if (sortType == SORT_BY_OCCURANCE) {
                    if (words[j].count < words[j + 1].count) {
                        swap(&words[j], &words[j + 1]);       //swaps the position if the current words.count is less than next words.count
                    }
                }
